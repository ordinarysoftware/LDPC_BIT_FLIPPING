...
var TannerGraph = function (matrix) {
  ...
  // Method to refresh the labels of all check nodes
  this.refreshCheckNodeLabels = function () {
    for (var i = 0; i < this.checkNodes.length; i++) {
      var n=this.symbolNodes.length;
      this.checkNodes[i].label = "C" + i + "\n Value: 0";
    }
  ...
  for (var i = 0; i < matrix.length; i++) {
    for (var j = 0; j < matrix[i].length; j++) {
      if (matrix[i][j] === 1) {
        ...
        this.edges.push({
          from: this.checkNodes[i].id,
          to: this.symbolNodes[j].id,
        });
      }
    }
  }
...
TannerGraph.prototype.getNode = function (id) {
  return (
    this.symbolNodes.concat(this.checkNodes).filter((n) => n.id === id)[0] ||
    null
  );
};
...
TannerGraph.prototype.getCheckNodeWithLowestDegree = function () {
  return this.checkNodes.reduce((lowest, current) => {
    if (current.connections.length < lowest.connections.length) return current;
    return lowest;
  }, this.checkNodes[0]);
};
...
SubGraph.prototype.coveredCheckNodes = function () {
  ...
  while (queue.length) {
    ...
    if (node.group === "check") coveredCheckNodes.push(node);
...
SubGraph.prototype.allCheckNodesCovered = function () {
  return (
    this.coveredCheckNodes().length === this._tannerGraph.checkNodes.length
  );
};
...
TannerGraph.prototype.render = function (container, onClick) {
  ...
  var data = {
    nodes: new TannerGraph.vis.DataSet(
      this.symbolNodes
        .map((symbolNode, index) => {
          symbolNode.physics = false;
          symbolNode.x =
            (index * container.offsetWidth) / (this.symbolNodes.length + 1);
          symbolNode.y = container.offsetHeight * 0.25;
          return symbolNode;
        })
        .concat(
          this.checkNodes.map((checkNode, index) => {
            checkNode.physics = false;
            checkNode.x =
              (index * container.offsetWidth) / (this.checkNodes.length + 1);
  ...
  if (onClick) network.on("doubleClick", onClick);
};
